...
MailRouter.post('/updateMailSystem', (0, express_validator_1.check)('mailId').exists({ checkFalsy: true, checkNull: true }).isLength({ min: 24, max: 24 }), (0, express_validator_1.check)('isActive').exists({ checkNull: true }), function (req, res) {
    let error = (0, express_validator_1.validationResult)(req);
    let errorList = error.array();
    if (errorList.length) {
        res.send({
